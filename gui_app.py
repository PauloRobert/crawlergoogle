...
def execute_spider():
    if dataset_entry.get() == '' or chosen_feed not in ['Json','Csv','xlsx']:
        messagebox.showerror('Error','All entries are required')
        return
    
    try:
        feed_uri = ("file:///%s/%s.%s"%(folder_path,dataset_entry.get(),chosen_feed))
    except:
        messagebox.showerror('Error','All entries are required')
...
feeds_dropdown.grid(row=1, column=1)
...
dataset_entry = Entry(app,textvariable = dataset_text)
...
execute_btn.grid(row=3,column=0,columnspan=3,pady=10)
